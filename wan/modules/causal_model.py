...
class CausalWanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.block_length = 4 * self.frame_length
        self.sink_size = max(1, sink_size)
        self.max_attention_size = 25 * self.frame_length
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            grid_sizes_one_block[:, 0] = self.sink_size

            # only caching the first block
            cache_write_len = min(self.block_length, k.shape[1])
            cache_end = cache_start + cache_write_len
            num_new_tokens = cache_end - kv_cache["global_end_index"].item()
            kv_cache_size = kv_cache["k"].shape[1]

            sink_tokens = self.sink_size * self.frame_length # first frame in cache
            ...
            if (num_new_tokens > 0) and (
                ...
                local_start_index = local_end_index - cache_write_len
                kv_cache["k"][:, local_start_index:local_end_index] = roped_key[:, :cache_write_len]
                kv_cache["v"][:, local_start_index:local_end_index] = v[:, :cache_write_len]
            else:
                ...
                if local_start_index == 0: # first block is not roped in the cache
                    kv_cache["k"][:, local_start_index:local_end_index] = k[:, :cache_write_len]
                else:
                    kv_cache["k"][:, local_start_index:local_end_index] = roped_key[:, :cache_write_len]
            ...
            if local_start_index == 0:
                ...
                if updating_cache: # updating working cache with clean frame
                    ...
                    if extract_cache_start == 0: # rope the global first block in working cache
                        working_cache_key[:, :sink_tokens] = causal_rope_apply(
                            working_cache_key[:, :sink_tokens], grid_sizes_one_block, freqs, start_frame=0).type_as(v)
                    ...
                    working_cache_max_length = self.max_attention_size - query_length - sink_tokens

                    extract_cache_end = local_start_index
                    extract_cache_start = max(sink_tokens, local_start_index - working_cache_max_length)
                    ...
                    rope_start_frame = current_start_frame - working_cache_frame_length - self.sink_size

                    anchor_cache_key = causal_rope_apply(
                        kv_cache["k"][:, :sink_tokens], grid_sizes_one_block, freqs, start_frame=rope_start_frame).type_as(v)
                    anchor_cache_v = kv_cache["v"][:, :sink_tokens]
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        if self.use_audio:
            ...
            self.audio_cond_projs = nn.ModuleList(
                [nn.Linear(audio_hidden_size, audio_out_dim) for _ in range(max(num_layers // 2 - 1, 0))]
            )
    ...
    def _forward_inference(
        ...
        # embeddings
        lat_h, lat_w = x[0].shape[-2], x[0].shape[-1]
        ...
        if audio_emb is not None and self.use_audio:
            audio_tokens = audio_emb.permute(0, 2, 1)[:, :, :, None, None]
            audio_tokens = torch.cat([audio_tokens[:, :, :1].repeat(1, 1, 3, 1, 1), audio_tokens], 2) if audio_tokens.size(2) % 4 != 0 else audio_tokens
            audio_tokens = self.audio_proj(audio_tokens)
            audio_tokens = torch.concat([proj(audio_tokens) for proj in self.audio_cond_projs], 0)
            # reshape to (B, num_proj, T, H, W, C) for per-layer injection
            audio_tokens = audio_tokens.reshape(
                x.shape[0],
                audio_tokens.shape[0] // x.shape[0],
                *audio_tokens.shape[1:],
            )
        else:
            audio_tokens = None
        for block_index, block in enumerate(self.blocks):
            if self.use_audio and audio_tokens is not None and (block_index <= len(self.blocks) // 2) and (block_index > 1):
                au_idx = block_index - 2
                audio_emb_tmp = audio_tokens[:, au_idx].repeat(1, 1, lat_h // 2, lat_w // 2, 1)
    ...
    def _forward_train(
        ...
        if audio_emb is not None and self.use_audio:
            audio_tokens = audio_emb.permute(0, 2, 1)[:, :, :, None, None]
            audio_tokens = torch.cat([audio_tokens[:, :, :1].repeat(1, 1, 3, 1, 1), audio_tokens], 2)
            audio_tokens = self.audio_proj(audio_tokens)
            audio_tokens = torch.concat([proj(audio_tokens) for proj in self.audio_cond_projs], 0)
            # reshape to (B, num_proj, T, H, W, C) for per-layer injection
            audio_tokens = audio_tokens.reshape(
        ...
        for ii, block in enumerate(self.blocks):
            if self.use_audio and audio_tokens is not None and (ii <= len(self.blocks) // 2) and (ii > 1):
                au_idx = ii - 2
                audio_emb_tmp = audio_tokens[:, au_idx].repeat(1, 1, lat_h // 2, lat_w // 2, 1)
                audio_cond_tmp = self.patchify(audio_emb_tmp.permute(0, 4, 1, 2, 3))[0]
                x = audio_cond_tmp + x
